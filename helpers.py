...
# Used for printing out the predictive power of a variable
...
# Uses the generate_iv_df method to generate a Pandas DataFrame
# containing all WoE and IV data for each Series inside the df
# using the second parameter as a target (dependent) variable.
...
# Uses the DataFrame created by create_intermediary_iv_df function
# to complete the calculation of WoE and IV based on the number of 
# Events and Non Events for two Pandas Series
...
# Sums up the number of events and non events in the binned_tuple_collection
# and creates a Pandas DataFrame with the following format:
# Bin Number, Non Events, Events
def create_intermediary_iv_df(s1: pd.Series, s2: pd.Series):
    ...
    for i in range (0, num_of_bins):
        ...
        yes_number = len(newDf[CONST_event_y_n].where(lambda x : x == CONST_y).dropna())
        no_number = total_num_rows - yes_number
        
        bins.append(i)
        non_events.append(no_number)
        events.append(yes_number)
...
# Creates a collection of tuples with the following format
# (Bin Number, First Bin Value, Second Bin Value, Event(Yes/No))
# Holds the main condition on whether two values are considered
# an Event (Good) or Non Event (Bad)
...
# A helper function used for converting the object generated by
# get_binned_tuple_of_series into a Pandas DataFrame

# NOTE: 
# If anything is changed in the algorithm that creates the binned_tuple_collection object,
