...
# Z Score
# - a way to compare results from a test to a “normal” population and
# it is divided into the following format:
# 1) below -3 
# 2) between -3 and 3
# 3) above 3
    
# Groups Z scores for an array generated by the generate_zscore_groups 
# function into 3 groups mentioned above
def group_zscores(zscore_group):
    less_than_minus_three = []
    between_minus_three_and_three = []
    greater_than_three = []
    for i in zscore_group:
        if (i < -3):
...
# Generating a Z Score for every Pandas Series
...
def print_iv_data(df: pd.DataFrame, dependentVariableName): 
    ...
    # array of variabels which have a 'Medium' or greater predicting power (of the dependent variable)
    decent_powered_variables = []
    for column in df:
    ...
    for i in range(0, len(iv_arr)):
        predictive_val = iv_arr[i][1]
        print(iv_arr[i][0] + ': ' + str(predictive_val) + '| Predicitive power: ' + group_predictive_pw(predictive_val))
        if (predictive_val > 0.1):
            decent_powered_variables.append(iv_arr[i][0]);
    
    print()
    print('Based on the values of the following variables, we have a significant '+
          'amount of chance to correctly predict the value of ' + dependentVariableName + ': ')
    for i in range(0, len(decent_powered_variables)):
        print(decent_powered_variables[i])
